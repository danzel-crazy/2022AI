...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        1.featurevals is the value of f(x), each row is for a haarfeature class
        2.if f(x) < 0 , h(x) = 1, else h(x) = 0
        3.use the error function in lecture2 to produce weights
        4.store error in the list - clf
        5.use  min find the minimal value of error, store in bestError
        6.use list.index find the index of bestError
        7.stor the specify features to bestClf 
        """
        clf = []
        for i in range(len(featureVals)):
            ...
            for j in range(len(featureVals[i])):
                if(featureVals[i][j] < 0):
                    error += weights[j] * abs(1-labels[j])
                else:
                    error += weights[j] * abs(0-labels[j])
            clf.append(error);
            
        bestError = min(clf)
        min_index = clf.index(bestError)
        print(bestError)
        print(min_index)
        bestClf = WeakClassifier(features[min_index]) 
